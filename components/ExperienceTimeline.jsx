...
const ExperienceCard = ({ experience, isDarkMode }) => {
  ...
  return (
    ...
    >
      ...
      >
        <h3
          className={`text-xl font-bold tracking-wide font-Ovo ${titleColorClass}`}
        >
          {experience.title}
        </h3>
        <p
          className={`text-base font-semibold !mt-1 font-Ovo ${companyColorClass}`}
          style={{ margin: 0 }}
        >
        ...
        <ul className="mt-5 list-disc ml-5 space-y-2">
          {experience.points.map((point, idx) => (
            <li
              key={idx}
              className={`text-[14px] pl-1 tracking-normal font-Ovo ${pointsColorClass}`}
            >
              {point}
            </li>
          ))}
  ...
  return (
    <section id="experience" className="py-16 md:py-24 scroll-mt-20">
      <div className="container mx-auto px-4 md:px-8">
        <h2 className="text-3xl md:text-4xl font-bold text-center mb-16 font-Ovo text-gray-800 dark:text-white">
          Experiences
        </h2>
        ...
        <VerticalTimeline lineColor={timelineLineColor}>
          ...
          <VerticalTimelineElement
            iconStyle={{ background: endIconBg, color: "#fff" }}
            icon={<FaStar />}
          />
        </VerticalTimeline>
      </div>
